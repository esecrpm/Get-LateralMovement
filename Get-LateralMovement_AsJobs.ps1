...
.NOTES
	...
	2022-02-14	0.4	Run commands as jobs to address slow processing over remote links
...
# Prepare for running as jobs and timing
$date1 = Get-Date
$jobs = @()
...
$jobs += Start-Job -ArgumentList $Tools,$SrcPath,$DstPath -ScriptBlock {
	param($Tools,$SrcPath,$DstPath)
...
$jobs += Start-Job -ArgumentList $Tools,$SrcPath,$DstPath -ScriptBlock {
	param($Tools,$SrcPath,$DstPath)
	Invoke-Expression "& '$Tools\RECmd\RECmd.exe' -d '$SrcPath' --bn '$Tools\RECmd\BatchExamples\Kroll_Batch.reb' --csv '$DstPath' --csvf LatMvmt_Registry.csv | Out-File -Encoding ASCII -FilePath '$DstPath\!RECmd_Messages.txt'"
}
Write-Host "- ShellBags"
If ($Live) {
	$jobs += Start-Job -ArgumentList $Tools,$SrcPath,$DstPath -ScriptBlock {
		param($Tools,$SrcPath,$DstPath)
		Invoke-Expression "& '$Tools\SBECmd.exe' -l --dedupe --csv '$DstPath' --csvf LatMvmt_ShellBags.csv | Out-File -Encoding ASCII -FilePath '$DstPath\!SBECmd_Messages.txt'"
	}
} else {
	$jobs += Start-Job -ArgumentList $Tools,$SrcPath,$DstPath -ScriptBlock {
		param($Tools,$SrcPath,$DstPath)
		Invoke-Expression "& '$Tools\SBECmd.exe' -d '$SrcPath\Users' --dedupe --csv '$DstPath' --csvf LatMvmt_ShellBags.csv | Out-File -Encoding ASCII -FilePath '$DstPath\!SBECmd_Messages.txt'"
	}
}
Write-Host "- ShimCache"
$jobs += Start-Job -ArgumentList $Tools,$SrcPath,$DstPath -ScriptBlock {
	param($Tools,$SrcPath,$DstPath)
	Invoke-Expression "& '$Tools\AppCompatCacheParser.exe' -f '$SrcPath\Windows\System32\config\SYSTEM' --csv '$DstPath' --csvf LatMvmt_ShimCache.csv | Out-File -Encoding ASCII -FilePath '$DstPath\!AppCompatCache_Messages.txt'"
}
Write-Host "- Amcache"
$jobs += Start-Job -ArgumentList $Tools,$SrcPath,$DstPath -ScriptBlock {
	param($Tools,$SrcPath,$DstPath)
	Invoke-Expression "& '$Tools\AmcacheParser.exe' -f '$SrcPath\Windows\appcompat\Programs\Amcache.hve' --csv '$DstPath' --csvf LatMvmt_Amcache.csv | Out-File -Encoding ASCII -FilePath '$DstPath\!Amcache_Source_Messages.txt'"
}
...
$jobs += Start-Job -ArgumentList $Tools,$SrcPath,$DstPath -ScriptBlock {
	param($Tools,$SrcPath,$DstPath)
	Invoke-Expression "& '$Tools\MFTECmd.exe' -f '$SrcPath\$MFT' --csv '$DstPath' --csvf LatMvmt_mft.csv | Out-File -Encoding ASCII -FilePath '$DstPath\!MFTECmd_Messages.txt'"
}
Write-Host "- Prefetch"
$jobs += Start-Job -ArgumentList $Tools,$SrcPath,$DstPath -ScriptBlock {
	param($Tools,$SrcPath,$DstPath)
	Invoke-Expression "& '$Tools\PECmd.exe' -d '$SrcPath\Windows\Prefetch' -q --csv '$DstPath' --csvf LatMvmt_Prefetch.csv | Out-File -Encoding ASCII -FilePath '$DstPath\!PECmd_Messages.txt'"
}
Write-Host "- JumpLists"
$jobs += Start-Job -ArgumentList $Tools,$SrcPath,$DstPath -ScriptBlock {
	param($Tools,$SrcPath,$DstPath)
	Invoke-Expression "& '$Tools\JLECmd.exe' -d '$SrcPath\Users' -q --csv '$DstPath' --csvf LatMvmt_JumpLists.csv | Out-File -Encoding ASCII -FilePath '$DstPath\!JLECmd_Messages.txt'"
}
Write-Host "- Shortcut Files"
$jobs += Start-Job -ArgumentList $Tools,$SrcPath,$DstPath -ScriptBlock {
	param($Tools,$SrcPath,$DstPath)
	Invoke-Expression "& '$Tools\LECmd.exe' -d '$SrcPath\Users' -q --neb --csv '$DstPath' --csvf LatMvmt_Shortcuts.csv | Out-File -Encoding ASCII -FilePath '$DstPath\!LECmd_Messages.txt'"
}
...
# Wait for Jobs to complete and clean up after
$jobs | Wait-Job | Receive-Job
$jobs | Remove-Job

# Calculate time to run script
$Date2 = Get-Date
Write-Host "`nTotal Time:"($date2-$date1).minutes"minutes,"($date2-$date1).seconds"seconds.`n"